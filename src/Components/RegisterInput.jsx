            ...
            <div className="register-form bg-obsidian-surface rounded-b-2xl pb-3">
                <form className='px-5 md:px-9 py-6' onSubmit={handleSubmit} method="post">
                    ...
                    <div className="submit-button mt-8">
                        <input type="submit" value="Criar Conta" className="w-full border-4 sm:border-6 border-champagne hover:border-white text-white font-black p-2 sm:p-3 px-4 sm:px-6 hover:bg-[#d28127] transition-colors cursor-pointer uppercase text-base sm:text-lg md:text-xl" />
                    </div>
                    {message && <p className="mt-3 text-center text-champagne">{message}</p>}
                    <div className="no-account mt-4 sm:mt-5 md:mt-6">
                        <p className="text-center font-bold text-xs sm:text-sm md:text-base text-[#454545] tracking-tighter">Já tem uma conta? <Link to="/login" className='text-champagne'>Entrar</Link></p>
                    </div>
                </form>
            </div>
        </div>
